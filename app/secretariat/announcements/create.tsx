...
export default function CreateAnnouncementScreen() {
  ...
  const handlePickImages = async () => {
    ...
    if (!result.canceled && result.assets) {
      // For now, we'll use the URIs directly
      // In production, you'd upload these to a storage service (Supabase Storage, Cloudinary, etc.)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        window.alert('✓ Announcement submitted for review successfully!');
      } else {
        Alert.alert(
          '✓ Success',
          'Your announcement has been submitted for review!',
          [{ text: 'OK' }]
        );
      }
